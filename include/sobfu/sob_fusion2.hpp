...
public:
    /* default constructor */
    SobFusion2(const Params &params);
    /* default destructor */
    ~SobFusion2();
    ...
    bool operator()(const std::vector<kfusion::cuda::Depth> &depths);

private:
    // std::vector<cv::Affine3f> poses_;
    std::vector<kfusion::cuda::Dists> dists_vec_;
    kfusion::cuda::Frame2 curr_;
    //, prev_;
